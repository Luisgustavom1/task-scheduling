...
from scheduler import Action, ScheduleTask, ScheduleTaskOnCores, Scheduler, TimeSpan, TransferData
...
class Runner:
  def __init__(self, node_id: int, wf: Workflow, scheduler: Scheduler, resources: List[Resource], config: Config = Config()):
    self.id = node_id
  ...
  def start(self) -> None:
    for data_id, data_item in enumerate(self.workflow.get_data_items()):
      if data_item.state == DataItemState.READY:
        ...
        self.resource_data_items.get(self.id, set()).add(data_id)
  ...
  def _process_actions(self) -> None:
    for i in range(len(self.resources)): 
      self._process_resource_queue(i);
  ...
  def _process_resource_queue(self, resource_idx: int):
  ...
  def _on_task_completed(self, task_id: int):
    ...
    self._process_actions()
    self._check_and_log_completed()

  def _check_and_log_completed(self):
